...
class AffectedModuleDetectorImpl constructor(
    ...
    override fun shouldInclude(project: Project): Boolean {
        ...
        val isInAffectedProjects = affectedProjects.contains(project)
        println("project: $project, affectedProjects: $affectedProjects, isInAffectedProjects: $isInAffectedProjects")
        println()
        return (
            (project.isRoot || (
                affectedProjects.contains(project) && isProjectProvided2(project))) && !config.excludedModules.contains(project.name)
            ).also {
            logger?.info(
    ...
    override fun isProjectProvided2(project: Project): Boolean {
        ...
        println("isProjectProvided2: $modules, path: ${project.path}, contains: ${modules.contains(project.path)}")
    ...
    private fun findAffectedProjects(): Set<Project> {
        ...
        if (projectSubset == ProjectSubset.CHANGED_PROJECTS) {
            println("ROMAN: logic = ProjectSubset.CHANGED_PROJECTS")
            return changedProjects
        }
        ...
        if (changedProjects.isEmpty() && unknownFiles.isEmpty()) {
            println("ROMAN: logic = buildAll = true because [1]: ${changedProjects.isEmpty()} && [2]: ${unknownFiles.isEmpty()}")
            buildAll = true
        }
        logger?.info(
